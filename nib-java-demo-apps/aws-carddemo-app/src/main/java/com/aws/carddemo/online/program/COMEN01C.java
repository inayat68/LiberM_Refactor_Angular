...
import com.aws.carddemo.online.program.storage.comen01c.*;
...
public class COMEN01C extends AbstractOnlineProgram {
  ...
  public COMEN01C(Context supernaut) {
    super(supernaut);
  }
  ...
  protected void buildMenuOptions() {
    ...
    for (;
      ...
      // COB(243): IF WS-IDX >= 1459 AND <= 1471 OR = 1101
      if (ws.wsVariables.wsIdx.greaterEqualThan(1459) && ws.wsVariables.wsIdx.lessEqualThan(1471)
          || ws.wsVariables.wsIdx.equals(1101)) {
        // COB(244): DISPLAY 'IS OK!'
        sysout.display("IS OK!");
        // COB(245): END-IF
      }
      // COB(246): IF CDEMO-MENU-OPT-NUM(WS-IDX) NUMERIC
      if (ws.comen02y
          .carddemoMainMenuOptions
          .cdemoMenuOptions
          ...
          .cdemoMenuOptNum
          .isNumeric()) {
        // COB(247): DISPLAY 'IS NUMERIC!'
        sysout.display("IS NUMERIC!");
        // COB(248): END-IF
      }
      // COB(249): STRING CDEMO-MENU-OPT-NUM(WS-IDX)  DELIMITED BY SIZE
      // COB(249):        '. '                         DELIMITED BY SIZE
      // COB(249):        CDEMO-MENU-OPT-NAME(WS-IDX) DELIMITED BY SIZE
      // COB(249):   INTO WS-MENU-OPT-TXT
