...
import com.aws.carddemo.online.program.storage.irisutil.*;
...
public class IRISUTIL extends AbstractProgram {
  ...
  // Linkage
  public static class IrisutilLinkage extends NGroup {
    ...
    public static class IrisDbPcb extends NGroup {
      ...
      public static class DbRntCntlArea extends NGroup {
        ...
        public static class Filler387 extends NGroup {
    ...
    public static class IrisMsgPcb extends NGroup {
    ...
    public static class IrisLkCells extends NGroup {
      ...
      public static class Filler403 extends NGroup {
    ...
    public static class LkDliuib extends NGroup {
      ...
      // COB:             05 UIBRCODE.
      public static class Uibrcode extends NGroup {
    ...
    public static class IrisInqyArea extends NGroup {
    ...
    public static class IrisIoPcb extends NGroup {
  ...
  protected int procedure(AbstractNField... args) throws ContextException {
    if (args != null) {
      params.allocate();
      if (args.length > 0){
        params.irisWorkArea.setAddress(args[0].getAddress());
      }
      if (args.length > 1){
        params.irisPcbPtr1x.setAddress(args[1].getAddress());
        params.irisLkDibBlock.setAddress(args[1].getAddress());
      }
      if (args.length > 2){
        params.irisPcbPtr2x.setAddress(args[2].getAddress());
      }
    }
    mainProgram();
    params.irisWorkArea.getAddress();
    args[0].getAddress();
    return RETURN_CODE;
  }
  ...
  protected void handleSched() {
    ...
    //  sql.setCommand(
    //          "SET search_path = darkside, vsam, public;");
    //  sql.execute();
    //}
    if (params.irisWorkArea.irisFuncSchd()) {
      ...
      //       *      SET IRIS-EXEC-DLI-PTR TO IRIS-LK-BLL-POINTER
    ...
    // COB(639): SET DB-STATUS-OK TO TRUE.
  ...
  protected void handleTerm() {
    // COB(669): MOVE 'HANDLE-TERM' TO WS-LAST-IORTN-SECTION
    ws.wsLastIortnSection.setString("HANDLE-TERM");
    // COB(672): MOVE LOW-VALUE TO IRIS-PCBS-AREA.
    //       *
  ...
  protected void handleBasicChkp() {
    // COB(689): MOVE 'HANDLE-BASIC-CHKP' TO WS-LAST-IORTN-SECTION
    ws.wsLastIortnSection.setString("HANDLE-BASIC-CHKP");
    // COB(691): EXEC SQL COMMIT END-EXEC.
    //       * DEMO EFFECT: NOT SAVING AREA AS NOT NEEDED FOR DEMOS
    // FIXME-[NOT_CONVERTED] EXEC :691 >COB: [           EXEC SQL COMMIT END-EXEC.]
  ...
  protected void finalSched() {
    // COB(703): MOVE 'FINAL-SCHED' TO WS-LAST-IORTN-SECTION
    ws.wsLastIortnSection.setString("FINAL-SCHED");
    // COB(706): PERFORM VARYING WS-INDEX FROM 1 BY 1
    // COB(706):         UNTIL WS-INDEX > IRIS-PARAM-NUM
    //       *
    //       *
    ws.wsIndex.setValue(1);
    for (; !ws.wsIndex.greaterThan(params.irisWorkArea.irisParamNum); ws.wsIndex.add(1)) {
      // COB(709): IF NOT IRIS-NO-ERROR
      //       *
      if (!params.irisWorkArea.irisNoError()) {
        // COB(710): DISPLAY 'IRISUTIL - ALLOCATE PSB ERROR'
        sysout.display("IRISUTIL - ALLOCATE PSB ERROR");
        // COB(711): GO TO FINAL-SCHED-EX
        return;
        // COB(712): END-IF
      }
      // COB(718): SET WS-PTR TO ADDRESS OF IRIS-PCBS-TAB(WS-INDEX)
      //       *
      ...
      ws.irisPcbsArea
          ...
          .runImsDualPointer
          .setValue(0);
      // COB(725): SET WS-DUAL-SQL-ONLY TO TRUE
      ws.setWsDualSqlOnly(true);
      // COB(726): MOVE WS-INDEX TO RUN-PCB-INDEX(WS-INDEX)
      ws.irisPcbsArea
          .irisPcbsTabAtIndex(ws.wsIndex.getInt() - 1)
          .irisRntCntlArea
          .runRunFixed
          .runPcbIndex
          .setValue(ws.wsIndex);
      // COB(727): MOVE WS-RUN-DBD-STATUS TO RUN-DBD-STATUS(WS-INDEX)
      ws.irisPcbsArea
          .irisPcbsTabAtIndex(ws.wsIndex.getInt() - 1)
          .irisRntCntlArea
          .runRunFixed
      ...
      // COB(728): MOVE IRIS-PSB-NAME TO DT-PSB-NAME(WS-INDEX)
      ws.irisPcbsArea
          .irisPcbsTabAtIndex(ws.wsIndex.getInt() - 1)
          .memoryPcbArea
          .dtPsbPcb
  ...
  protected void handleBatchPcb() {
    // COB(741): MOVE 'HANDLE-BATCH-PCB' TO WS-LAST-IORTN-SECTION
    ws.wsLastIortnSection.setString("HANDLE-BATCH-PCB");
    // COB(743): PERFORM VARYING WS-INDEX FROM 1 BY 1
    // COB(743):   UNTIL WS-INDEX > IRIS-PARAM-NUM
    // COB(743):      OR IRIS-PCB-TYPE-DB(WS-INDEX)
    // COB(743):      OR IRIS-PCB-TYPE-GSAM(WS-INDEX)
    //       *
    ws.wsIndex.setValue(1);
    for (;
    ...
    // COB(749): MOVE SPACE TO IRIS-AUDIT-FIELDS
    //       *
    params.irisWorkArea.irisAuditFields.spaces();
    // COB(750): MOVE LOW-VALUE TO IRIS-PCBS-AREA
    ws.irisPcbsArea.lowValues();
    // COB(751): MOVE IRIS-PARAM-NUM TO WS-SCHED-PARAM-NUM
    ws.wsSchedParamNum.setValue(params.irisWorkArea.irisParamNum);
    // COB(752): MOVE IRIS-PSB-NAME TO DT-PSB-NAME(1)
    ws.irisPcbsArea
        ...
        .dtPsbName
        .setValue(params.irisWorkArea.irisPsbName);
    // COB(754): MOVE ZERO TO WS-INDEX
    //       *
    ws.wsIndex.setValue(0);
    // COB(755): MOVE ZERO TO WS-INDEX-C
    ws.wsIndexC.setValue(0);
    // COB(759): SET IRISADDR-RTN TO TRUE
    //       *
    //       *    IRIS PCB ALLOCATION
    //       *
    ws.iriscomm.setIrisaddrRtn(true);
    // COB(761): PERFORM HANDLE-BATCH-PCB-A THRU HANDLE-BATCH-PCB-A-EX
    //       *
    handleBatchPcbA();
    // COB(766): SET IRIS-EXEC-DLI-PTR TO ADDRESS OF IRIS-BLL-CELLS
    //       *
    //       *    SET IRISADDR-RTN TO TRUE
    //       *    CALL IRIS-WS-RTN USING IRIS-WORK-AREA IRIS-BLL-CELLS
    //       *                           IRIS-EXEC-DLI-PTR
  ...
  protected void handleBatchPcbA() {
    // COB(775): PERFORM HANDLE-BATCH-PCB-A1
    handleBatchPcbA1();
    // COB(776): PERFORM HANDLE-BATCH-PCB-A2
    handleBatchPcbA2();
    // COB(777): PERFORM HANDLE-BATCH-PCB-A3
    handleBatchPcbA3();
    // COB(778): PERFORM HANDLE-BATCH-PCB-A4
    handleBatchPcbA4();
    // COB(779): PERFORM HANDLE-BATCH-PCB-A5
    handleBatchPcbA5();
  }
  ...
  protected void handleBatchPcbA1() {
    ...
    if (!ws.wsIndex.greaterThan(params.irisWorkArea.irisParamNum)
      ...
      //       *
      //       *      DISPLAY 'IRIS-PCB-PTR-' WS-INDEX '=' IRIS-PCB-PTR-:NPCB:
      //       *      UPON CONSOLE
      //       *
      if (params.irisWorkArea.irisFinal()) {
        ...
        //       *        SET IRIS-PCB-PTR-:NPCB: TO WS-PTR
        //       *        SET ADDRESS OF IRIS-DB-PCB TO IRIS-PCB-PTR-:NPCB:
        ...
        if (params.irisWorkArea.irisUsedPcb.greaterThan(0)) {
          ...
          //       *        Program use less PCBs than scheduled
  ...
  protected void extractPcbExec() {
    // COB(980): MOVE 'EXTRACT-PCB-EXEC' TO WS-LAST-IORTN-SECTION
    ws.wsLastIortnSection.setString("EXTRACT-PCB-EXEC");
    // COB(982): SET ADDRESS OF LK-DIB-BLOCK TO ADDRESS OF IRIS-LK-DIB-BLOCK
    //       *
    params.lkDibBlock.setAddress(params.irisLkDibBlock.getAddress());
    // COB(983): MOVE LK-DIB-BLOCK TO IRIS-DIB-BLOCK
    ws.iriscomm.irisDibBlock.setValue(params.lkDibBlock);
    // COB(984): IF IRIS-BLL-POINTER(1) IS NOT NULL
    if (!ws.irisBllCells.irisBllCellAtIndex(0).irisBllPointer.equals(0)) {
      // COB(985): SET ADDRESS OF IRIS-DB-PCB TO IRIS-BLL-POINTER(1)
